...
// this will include the SIB, deduced NVM version, and silicon revision in early responses
// to SET_DEVICE_DESCRIPTPOR (for SIB and NVM version) 
// and ENTER_PROGMODE (for silicon rev)
...
/* User Configuration goes up here - the rest down below is defaults that you can override
  ...
  // You should be able to hit 2MBaud with F_CPU=16000000 and DEBUG_BAUDVAL 0 or 1mbaud with 1
  // if your serial adapter can't handle that.
  // At 20, though, on classic AVRs, you're out of luck unless you have an adapter that can do 2.5 mbaud...
  // have to go all the way down to 500kbaud (DEBUG_BAUDVAL 4) to divide it to something common..
  // Good thing almost everyone runs them at 16 MHz.
  ...
  // Same deal as before/ Remember that the buildin LED is on the same pin as SCK on most boards,
  // so you'll want to cadd LEDs on other pinn, defaults to D2
...
// big difference here is your specify the name of the peripheral instead of the number,
// and the target baud rate, because we grab the OSCCAL value per datasheet./
//  #define USE_USARTDEBUG
...
#else
  #error "Part not supported - if you didn't provide all the needed pin definitions, that's why it's not compiling"
  #include "NOSUPPORTED_BUILD_STOP"
...
// Some classic AVRs have a single flags register for timers 0 and 1
// (TIFR0 and TIFR1 are merged in a single register TIFR)
#ifndef XAVR
  #ifndef TIFR1
    #define TIFR1 TIFR
  #endif
 ...
 *                              Only the OC0A pin can be used for UPDI I/O. Slightly faster upload speed
 *                              for a given UPDI_BAUD value.
 *
